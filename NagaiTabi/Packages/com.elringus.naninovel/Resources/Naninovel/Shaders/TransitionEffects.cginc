...
inline float4 Crossfade(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress)
{
    const float4 CLIP_COLOR = float4(0, 0, 0, 0);
    float4 mainColor = PremultiplyAlpha(Tex2DClip01(mainTex, mainUV, CLIP_COLOR));
...
inline float4 BandedSwirl(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float twistAmount, float frequency)
{
...
inline float4 Blinds(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float count)
{
...
inline float4 CircleReveal(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float fuzzyAmount)
{
...
inline float4 CircleStretch(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress)
{
    float2 center = float2(0.5, 0.5);
...
inline float4 CloudReveal(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, sampler2D cloudsTex)
{
...
inline float4 Crumble(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float randomSeed, sampler2D cloudsTex)
{
...
inline float4 Dissolve(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float step)
{
    const float4 CLIP_COLOR = float4(0, 0, 0, 0);
...
inline float4 DropFade(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float randomSeed, sampler2D cloudsTex)
{
    const float4 CLIP_COLOR = float4(0, 0, 0, 0);
...
inline float4 LineReveal(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float fuzzyAmount, float2 lineNormal, float reverse)
{
...
inline float4 Pixelate(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress)
{
    float pixels;
    ...
    float4 mainColor = PremultiplyAlpha(tex2D(mainTex, newMainUV));
    float4 transitionColor = PremultiplyAlpha(tex2D(transitionTex, newTransitionUV));

    float lerpProgress = saturate((progress - 0.4) / 0.2);
    return lerp(mainColor, transitionColor, lerpProgress);
}

inline float4 RadialBlur(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress)
{
    float2 center = float2(0.5, 0.5);
    float2 toUV = mainUV - center;
...
inline float4 RadialWiggle(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float randomSeed, sampler2D cloudsTex)
{
    float2 center = float2(0.5, 0.5);
    float2 toUV = mainUV - center;
...
inline float4 RandomCircleReveal(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float randomSeed, sampler2D cloudsTex)
{
    float radius = progress * 0.70710678;
...
inline float4 Ripple(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float frequency, float speed, float amplitude)
{
    float2 center = float2(0.5, 0.5);
    float2 toUV = mainUV - center;
    float distanceFromCenter = length(toUV);
...
inline float4 RotateCrumble(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float randomSeed, sampler2D cloudsTex)
{
...
inline float4 Saturate(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress)
{
    float4 mainColor = PremultiplyAlpha(tex2D(mainTex, mainUV));
...
inline float4 Shrink(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float speed)
{
    const float4 CLIP_COLOR = float4(0, 0, 0, 0);
    float2 center = float2(0.5, 0.5);
    float2 toUV = mainUV - center;
...
inline float4 SlideIn(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float2 slideAmount)
{
...
inline float4 SwirlGrid(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float twistAmount, float cellCount)
{
...
inline float4 Swirl(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float twistAmount)
{
    const float4 CLIP_COLOR = float4(0, 0, 0, 0);
    float2 center = float2(0.5, 0.5);
    float2 toUV = mainUV - center;
...
inline float4 Water(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float randomSeed, sampler2D cloudsTex)
{
    float2 offset = tex2D(cloudsTex, float2(mainUV.x / 10, frac(mainUV.y / 10 + min(0.9, randomSeed)))).xy * 2.0 - 1.0;
...
inline float4 Waterfall(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float randomSeed, sampler2D cloudsTex)
{
...
inline float4 Wave(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float magnitude, float phase, float frequency)
{
    const float4 CLIP_COLOR = float4(0, 0, 0, 0);
...
inline float4 Custom(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, sampler2D customTex, float fuzzy, float invert)
{
    float4 mainColor = PremultiplyAlpha(tex2D(mainTex, mainUV));
    float4 transitionColor = PremultiplyAlpha(tex2D(transitionTex, transitionUV));
...
inline float4 ApplyTransitionEffect(sampler2D mainTex, float2 mainUV, sampler2D transitionTex, float2 transitionUV, float progress, float4 params, float2 randomSeed, sampler2D cloudsTex, sampler2D customTex)
{
