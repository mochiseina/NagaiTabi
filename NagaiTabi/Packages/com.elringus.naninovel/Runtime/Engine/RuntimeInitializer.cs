...
namespace Naninovel
{
    ...
    public class RuntimeInitializer : MonoBehaviour
    {
            ...
            IEnumerable<ServiceInitializationData> customInitializationData = null, ITime time = null)
        {
            ...
            if (!fastInitRequested) await PlayTitleScript();
        ...
        private static async Awaitable PlayTitleScript ()
        {
            var scriptPath = Engine.GetConfiguration<ScriptsConfiguration>().TitleScript;
            if (string.IsNullOrEmpty(scriptPath)) return;
            var player = Engine.GetServiceOrErr<IScriptPlayer>();
            await player.MainTrack.LoadAndPlay(scriptPath);
        }
